...
def process_class_photo(image_file, class_session_id):
    ...
    try:
        ...
        try:
            ...
            return False, f"Failed to save processed image: {str(e)}"
        
        # Update class session attendance stats
        ...
        processed_image = ProcessedImage(
            ...
            faces_recognized=present_students
        )
        db.session.add(processed_image)
        db.session.commit()
        
        return True, {
            "message": f"Successfully processed image with {present_students} students recognized",
            "faces_detected": len(face_locations),
            "students_recognized": present_students,
...
def process_multiple_photos(image_files, class_session_id):
    ...
    for image_file in image_files:
        success, result = process_class_photo(image_file, class_session_id)
        results.append({
